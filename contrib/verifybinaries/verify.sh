#!/bin/bash
# Copyright (c) 2016 The Bitzeny Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   bitzenycore.org and bitzeny.org and compares them.
...
WORKINGDIR="/tmp/bitzeny_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitzenycore.org"
HOST2="https://bitzeny.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitzeny-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitzeny-',
...
if [ $? -ne 0 ]; then
   echo "bitzeny.org failed to provide signature file, but bitzenycore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitzeny.org and bitzenycore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bitzeny Core binary release signing key installed?"
   fi
