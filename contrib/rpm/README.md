...
The RPM spec file provided here is for Bitzeny-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitzeny.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bitzeny git repository](https://github.com/bitzeny/bitzeny) but are not part of
...
Currently only `Source10` is used, for the example `bitzeny.conf` file.
...
in the bitzeny git repository. At present only an SVG version of the Bitzeny
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitzeny
...
needed when 0.12.1 is released, a proper fix is already in the Bitzeny git
master branch.
...
idea to build Bitzeny Core against a static version of that library compiled
...
The `bitzeny.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitzeny-server` package. At this point, most RPM based
...
The `bitzeny-server` package should have SELinux support. How to properly do
...
Bitzeny-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitzeny/). He has been
packaging Bitzeny for Fedora at least since 2012.
...
4. I renamed the `bitzeny` package that contains the Qt GUI to `bitzeny-core` as
that appears to be how the general population refers to it, in contrast to
`bitzeny-xt` or `bitzeny-classic`. I wanted to make sure the general population
