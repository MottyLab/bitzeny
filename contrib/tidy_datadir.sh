#!/bin/bash
# Copyright (c) 2013 The Bitzeny Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Bitzeny database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Bitzeny datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Bitzeny datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Bitzeny 0.7 datadir."
    ;;
  3)
    echo "Detected Bitzeny pre-0.8 datadir."
    ;;
  4)
    echo "Detected Bitzeny 0.8 datadir."
    ;;
esac
